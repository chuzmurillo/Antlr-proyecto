    ...
    var cpu = {
        step: function() {
            ...
            try {
                ...
                var checkOperation = function(value) { //ver el block de notas.
                    self.zero = false;
                    self.carry = false;
                ...
                switch(instr) {
                    ...
                    case opcodes.MOV_ADDRESS_TO_REG:
                        ...
                        self.ip++;
                        break;
                    case opcodes.MOV_REGADDRESS_TO_REG:      //-----------------> mov a, [c];
                        regTo = checkGPR_SP(memory.load(++self.ip));
                    ...
                    //    regTo = memory.load(++self.ip);
                    //    regFrom = checkGPR_SP(memory.load(++self.ip));
                    //    memory.store(self.display++, getGPR_SP(regFrom));
                    //    //alert(232+" --" +getGPR_SP(regFrom));//------------> MOv [D],A; 
                    //    self.ip++;
                    //    break;
                    case opcodes.MOV_NUMBER_TO_REG:
                       ...
                       // alert(regTo+' -'+number); //------------> mov c , helloword;
                        self.ip++;
                        break;
                    ...
                    //    self.ip++;
                    //    break;
					case opcodes.PRINT_STRING: //nuevo
					    number = memory.load(++self.ip); //pos de inico de string en memoria
					    character=-1;
						while(character!==0){
							character = memory.load(number++);
							memory.store(self.display++, character);
						}
						--self.display; // quitar espacio en blanco entre prints
						self.ip++;
                        break;
					case opcodes.PRINT_NUMBER: //nuevo
						number = checkGPR_SP(memory.load(++self.ip)); //se obtiene numero de registro(..a=1, b=2, ..)
						character = getGPR_SP(number).toString() + 'x'; //se obtiene el numero que contiene ese regitro
						number=0;
						while(character[number]!=='x'){
							memory.store(self.display++, character.charCodeAt(number++));
						}
						self.ip++;
                        break;
                    default:
